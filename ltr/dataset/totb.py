...
class Totb(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None, obj_classes=None):
        ...
        """
        root = env_settings().totb_dir if root is None else root
        super().__init__('Totb', root, image_loader)
    ...
    def _build_sequence_list(self, vid_ids=None, split=None, obj_classes=None):
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'totb_train_split.txt')
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'totb_val_split.txt')
            elif split == 'test':
                file_path = os.path.join(ltr_path, 'data_specs', 'totb_test_split.txt')
            else:
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, 'img', 'img{:05}.jpg'.format(frame_id+1))    # frames start from 1

    def _get_frame(self, seq_path, frame_id):
        return self.image_loader(self._get_frame_path(seq_path, frame_id))

    def _get_class(self, seq_path):
        raw_class = seq_path.split('/')[-2] if '/' in seq_path else seq_path.split('\\')[-2]
        return raw_class
