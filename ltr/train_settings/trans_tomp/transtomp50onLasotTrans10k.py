...
from ltr.dataset.trans10k_seq import Trans10kSeq
...
def run(settings):
    ...
    # Train datasets
    if not settings.personal_pc:
        print('Using LaSOT')
        lasot_train = Lasot(settings.env.lasot_dir, split='train')

    trans10k = Trans10kSeq(settings.env.trans10k_dir, split='train')
    # got10k_train = Got10k(settings.env.got10k_dir, split='vottrain')
    # trackingnet_train = TrackingNet(settings.env.trackingnet_dir, set_ids=list(range(4)))
    # coco_train = MSCOCOSeq(settings.env.coco_dir)
    # totb_train = Totb(split='train')
    totb_val = Totb(split='val')
    ...
    dataset_train = sampler.DiMPSampler([lasot_train, trans10k] if not settings.personal_pc else [trans10k], [1, 1] if not settings.personal_pc else [1],
