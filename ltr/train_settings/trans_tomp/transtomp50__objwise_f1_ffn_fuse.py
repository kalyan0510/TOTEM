...
def run(settings):
    ...
    # Train datasets
    if platform.uname().node == 'kalyans-galaxybook-pro':
        lasot_train = Totb(obj_classes=['Beaker', 'GlassBall', 'WubbleBubble'])
    else:
        lasot_train = Lasot(settings.env.lasot_dir, split='train')    # got10k_train = Got10k(settings.env.got10k_dir, split='vottrain')
